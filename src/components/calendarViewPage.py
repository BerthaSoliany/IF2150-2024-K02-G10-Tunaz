...
def calendar_view_page(page: ft.Page):
    ...
    sampai_tanggal = ft.CupertinoTextField(width=None, content_padding=5, border_radius=5, border=ft.border.all(1,"#D7D7D7"), bgcolor="white", placeholder_text=datetime.strptime(last_date,"%Y-%m-%d").strftime("%d/%m/%Y"), placeholder_style=ft.TextStyle(color=ft.Colors.GREY_600), text_style=ft.TextStyle(color="black"), read_only=True)
    if(count==1):
        last_date = None
        sampai_tanggal.placeholder_text = ""
        
    waktu_tanggal = ft.CupertinoTextField(width=None, content_padding=5, border_radius=5, border=ft.border.all(1,"#D7D7D7"), bgcolor="white", placeholder_text=datetime.strptime(jadwal_perawatan[5].split()[0], "%Y-%m-%d").strftime("%d/%m/%Y"), placeholder_style=ft.TextStyle(color=ft.Colors.GREY_600), text_style=ft.TextStyle(color="black"), read_only=True)
    waktu_jam = ft.CupertinoTextField(width= None, content_padding=5, border_radius=5, border=ft.border.all(1,"#D7D7D7"), bgcolor="white", placeholder_text=jadwal_perawatan[5].split()[1], placeholder_style=ft.TextStyle(color=ft.Colors.GREY_600), text_style=ft.TextStyle(color="black"), read_only=True)
    frekuensi = ft.CupertinoTextField(width=120, content_padding=5, border_radius=5, border=ft.border.all(1,"#D7D7D7"), bgcolor="white", placeholder_text=jadwal_perawatan[4], placeholder_style=ft.TextStyle(color=ft.Colors.GREY_600), text_style=ft.TextStyle(color="black"), read_only=True)
    frekuensi1 = ft.Row(controls=[frekuensi, ft.Text("hari", size=20, color="black")], expand=True)
    kebutuhan_perawatan_field = ft.CupertinoTextField(border_radius=5, border=ft.border.all(1,"#D7D7D7"),bgcolor="white", placeholder_text=tanaman.get_data_informasi_tanaman().get_kebutuhan_perawatan(), placeholder_style=ft.TextStyle(color=ft.Colors.GREY_600), text_style=ft.TextStyle(color="black"), read_only=True, multiline=True, min_lines=3, max_lines=3)
